        ...
        <nav className="hidden items-center gap-6 sm:flex">
          ...
          {user ? (
            <div className="relative">
              <button
                onClick={() => setMenu(v => !v)}
                className="inline-flex items-center gap-2 rounded-full border border-black/10 px-2 py-1 text-sm font-medium text-black transition hover:bg-zinc-100 dark:border-white/20 dark:text-white dark:hover:bg-white/10"
                title={user.email}
              >
                <span className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-amber-500/80 text-xs font-bold text-white">
                  {displayName.slice(0,1).toUpperCase()}
                </span>
                <span className="hidden sm:inline">{displayName}</span>
              </button>
              {menu && (
                <div className="absolute right-0 mt-2 w-40 overflow-hidden rounded-xl border border-black/10 bg-white text-sm shadow-lg dark:border-white/10 dark:bg-zinc-900">
                  <Link href="/profile" className="block px-3 py-2 hover:bg-zinc-50 dark:hover:bg-zinc-800">Profile</Link>
                  <form action={async () => {
                    const { logoutAction } = await import("@/app/actions/auth");
                    await logoutAction();
                  }}>
                    <button type="submit" className="block w-full px-3 py-2 text-left hover:bg-zinc-50 dark:hover:bg-zinc-800">Log out</button>
                  </form>
                </div>
              )}
            </div>
